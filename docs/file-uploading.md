...
Endpoint `/api/v1/files/upload` is used for uploading files. In this case `/api/v1/files/upload` receives only `fileName` property (without binary file), and returns the `presigned URL` and `key`. After receiving the `presigned URL` you need to upload your file directly to S3, then call `POST /api/v1/files/confirm` with the `key` to scan and commit the file.
...
sequenceDiagram
    ...
    A->>B: Confirm upload via POST /api/v1/files/confirm (key)
...
sequenceDiagram
    ...
    A->>B: POST /api/v1/files/confirm (key)
