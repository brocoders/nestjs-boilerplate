...
module.exports = {
  prompt: async ({ prompter, args }) => {
        ...
        .then(
          collectPromisesResults((values) => {
            ...
            return prompter.prompt({
              ...
              name: 'shouldAutoLoad',
              message:
                'Auto-load the related entity (autopopulate)? Choose No to store only the id.',
              initial: true,
            });
          }),
        );
    ...
    if (
      ...
      execSync(
        `npm run add:property:to-document -- --name ${result.type} --property ${result.propertyInReference} --propertyInReference ${result.property} --kind ${result.kind} --type ${result.name} --referenceType manyToOne --isAddToDto ${result.isAddToDto} --isOptional false --isNullable false --shouldAutoLoad false`,
        {
          stdio: 'inherit',
        },
      );
